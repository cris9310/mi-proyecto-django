...
class Dashboard(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for i in all_events_user:
            rr = {'id': i.id, 'title': i.title, 'descripcion': i.descripcion,
                  'fecha_inicial': i.start_time, 'fecha_final': i.end_time, 'color': i.color}
            user_id.append(rr)
    ...
    def get_queryset(self):
        ...
        if Estudiante.objects.filter(is_active=True).count() != 0:
            mujeres = round((Estudiante.objects.filter(is_active=True, sexo="Femenino").count(
            )/Estudiante.objects.filter(is_active=True).count())*100, 0)
            hombres = round((Estudiante.objects.filter(is_active=True, sexo="Masculino").count(
            )/Estudiante.objects.filter(is_active=True).count())*100, 0)
            otros = round((Estudiante.objects.filter(is_active=True, sexo="Otros").count(
            )/Estudiante.objects.filter(is_active=True).count())*100, 0)
            calculo = {'mujeres': str(mujeres), 'hombres': str(
                hombres), 'otros': str(otros)}
            estudiante_calc.append(calculo)
        else:
            mujeres = round((Estudiante.objects.filter(
                is_active=True, sexo="Femenino").count()/1)*100, 0)
            hombres = round((Estudiante.objects.filter(
                is_active=True, sexo="Masculino").count()/1)*100, 0)
            otros = round((Estudiante.objects.filter(
                is_active=True, sexo="Otros").count()/1)*100, 0)
            calculo = {'mujeres': str(mujeres), 'hombres': str(
                hombres), 'otros': str(otros)}
            estudiante_calc.append(calculo)
...
class CrearPensum(FormView):
    ...
    def form_valid(self, form):
        ...
        if Pensum.objects.filter(malla=malla, programa=programa):
            messages.warning(
                self.request, 'El pensum que intenta crear ya existe para este programa, por favor Verifique y vuelva a intentarlo.')
            return HttpResponseRedirect(self.success_url)
    ...
    def get_queryset(self):
        ...
        if kword:
            for i in Materias.objects.filtrar_buscador_Teacher_topic(kword, cod):
                try:
                    info = Banner.objects.values("materia__codigo").annotate(
                        total=Coalesce(Count("materia"), 0)).filter(materia__codigo=i.codigo)
                    info1 = {"pk": i.pk, "codigo": i.codigo, "programa": i.programa,
...
class TeacherNotesListview(ListView):
    ...
    def get_queryset(self):
        ...
        kword = self.request.GET.get('kword', '')
        if kword:
            queryset = Banner.objects.filtrar_buscador_Teacher_notes(
                kword, cod)
        else:
...
class AsignamateriaPreviewView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['pensum'] = Estudiante.objects.get(
            id=self.kwargs['pk']).pensum_asig
        context['periodo'] = Estudiante.objects.get(
            id=self.kwargs['pk']).periodo_matriculado
        return context
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        periodo_asig = Estudiante.objects.get(
            id=id_estudiante).periodo_matriculado
        sede_asig = Estudiante.objects.get(id=id_estudiante).sede
        queryset1 = Materias.objects.filter(
            programa_id=alumno.id, pensum_asig_id=pensum_asig.id, periodo_id=periodo_asig.id, materia_id=sede_asig.id)
    ...
    def get_queryset(self):
        ...
        if kword or order:
            queryset = Estudiante.objects.filtrar_student(
                kword, order).exclude(masivo=False)
        else:
            queryset = Estudiante.objects.filter(
...
class ListFinalily(ListView):
    ...
    def get_queryset(self):
        estudiante_list = []
        aceptado = Banner.objects.filter(promedio__gt=2.99).values(
            'cod_student').annotate(total=Count('cod_student')).order_by('total')
        ...
        for i in list(aceptado):
            ...
            if Graduated.objects.filter(student_id=st.pk).count() == 0:
                ...
                pagadas = Facturas.objects.filter(
                    user_id=st.codigo, estado_id=2).count()
                rr = {'pk': st.pk, 'codigo': st.codigo, 'estudiante': st.apellidos + ' ' + st.nombre, 'programa': carrera.programa_name,
                      'indicador': int((int(i['total'])/int(carrera.aceptado))*100), 'financiero': int((int(pagadas)/(2 + int(carrera.cuotas)))*100)}
...
class GraduateView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for i in studen_graduate:
            ...
            individual = Graduated(student_id=int(i), carrera_id=int(consulta.carrera_id), periodo_grado=str(
                date.year) + "-" + Graduated.objects.mes(date), libro=studen_libro, folio=studen_folio)
            todos.append(individual)
            Estudiante.objects.filter(id=int(i)).update(is_active=False)
        Graduated.objects.bulk_create(todos)
        messages.success(
            self.request, 'Perfecto, graduaste estudiantes de manera exitosa')
        return HttpResponseRedirect(
...
class TeacherDeleteFinalView(View):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if consulta == 0:
            ...
            print(j)
            if j > 0:
                object1 = Estudiante.objects.filter(
                    codigo=pk).update(is_active=False)
                object2 = User.objects.filter(pk=pk).update(is_active=False)
            else:
                object1 = Estudiante.objects.filter(codigo=pk).delete()
                object2 = User.objects.filter(pk=pk).delete()
                object3 = Banner.objects.filter(
                    cod_student_id=consulta1.id).delete()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if materias[0] != "":
            ...
            Estudiante.objects.filter(
                id=int(estudiante)).update(is_matriculado=True)
            return HttpResponseRedirect(
                reverse(
                    'academico_app:list-student'
                )
            ...
            messages.warning(
                self.request, 'Por Favor seleccione por lo menos una materia para asignar al estudiante')
            return HttpResponseRedirect(
                self.request.META.get("HTTP_REFERER")
...
class AddMateriasList(ListView):
    ...
    def get_queryset(self):
        ...
        try:
            materias_final = Materias.objects.filter(
                periodo_id=data_estudiante[0][2], programa_id=data_estudiante[0][1], id__in=[x for x in data_materias])
        except:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data == "generar":
            ...
            if data == "generar" and data_cierre.Cerrado == True:
                ...
                Habilitaciones.objects.create(
                    C1=True, C2=False, C3=False, Cerrado=False, B1=False, B2=False, B3=False, CF=False)
                Acciones.objects.create(usuario=str(informacion.username), nombre=str(
                    informacion.nombres) + " " + str(informacion.apellidos), accion="Ha generado nuevos cortes")
                ...
                messages.warning(
                    self.request, 'No puede generar nuevos periodos teniendo aun periodos activos')
                return HttpResponseRedirect(
                    reverse(
                        'academico_app:habilitado'
                    )
            ...
            if data_cierre.C1 == True and data == "C1" and data_cierre.B1 == False:
                Habilitaciones.objects.filter(
                    id=data_cierre.id).update(C1=False, B1=True)
                Acciones.objects.create(usuario=str(informacion.username), nombre=str(
                    informacion.nombres) + " " + str(informacion.apellidos), accion="Ha habilitado el corte 1")
                return HttpResponseRedirect(
                    reverse(
                        'academico_app:habilitado'
                    )
                ...
                Habilitaciones.objects.filter(
                    id=data_cierre.id).update(C1=True, C2=True, B1=True)
                Acciones.objects.create(usuario=str(informacion.username), nombre=str(
                    informacion.nombres) + " " + str(informacion.apellidos), accion="Ha cerrado el corte 1")
                return HttpResponseRedirect(
                    reverse(
                        'academico_app:habilitado'
                    )
            ...
            if data_cierre.C2 == True and data == "C2" and data_cierre.B2 == False:
                Habilitaciones.objects.filter(
                    id=data_cierre.id).update(C2=False, B2=True)
                Acciones.objects.create(usuario=str(informacion.username), nombre=str(
                    informacion.nombres) + " " + str(informacion.apellidos), accion="Ha habilitado el corte 2")
                ...
                Habilitaciones.objects.filter(
                    id=data_cierre.id).update(C2=True, C3=True, B2=True)
                Acciones.objects.create(usuario=str(informacion.username), nombre=str(
                    informacion.nombres) + " " + str(informacion.apellidos), accion="Ha cerrado el corte 2")
                return HttpResponseRedirect(
                    reverse(
                        'academico_app:habilitado'
                    )
            ...
            if data_cierre.C3 == True and data == "C3" and data_cierre.B3 == False:
                Habilitaciones.objects.filter(
                    id=data_cierre.id).update(C3=False, B3=True)
                Acciones.objects.create(usuario=str(informacion.username), nombre=str(
                    informacion.nombres) + " " + str(informacion.apellidos), accion="Ha habilitado el corte 3")
                ...
                Habilitaciones.objects.filter(
                    id=data_cierre.id).update(C3=True, CF=True, B3=True)
                Acciones.objects.create(usuario=str(informacion.username), nombre=str(
                    informacion.nombres) + " " + str(informacion.apellidos), accion="Ha cerrado el corte 3")
                return HttpResponseRedirect(
                    reverse(
                        'academico_app:habilitado'
                    )
            ...
            if data == "CF":
                Habilitaciones.objects.filter(
                    id=data_cierre.id).update(CF=True, Cerrado=True)
                Banner.objects.filter(materia__is_active=True).update(
                    promedio=(F('corte1')+F('corte2')+F('corte3'))/3)
                Materias.objects.filter(is_active=True).update(
                    is_active=False, updated_at=datetime.now())
                Acciones.objects.create(usuario=str(informacion.username), nombre=str(
                    informacion.nombres) + " " + str(informacion.apellidos), accion="Ha cerrado el semestre")
                return HttpResponseRedirect(
                    reverse(
                        'academico_app:habilitado'
                    )
...
class UpdateNotasView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Validación 2: Verificamos datos del archivo
            ...
            nota_student = pd.read_excel(
                nota_student, sheet_name="Plantilla", engine='openpyxl')
